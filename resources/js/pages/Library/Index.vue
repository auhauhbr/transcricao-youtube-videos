        ...
        <section class="flex-1 border-b border-border bg-background">
            <div class="mx-auto max-w-7xl px-4 py-7 sm:px-8 sm:py-10 lg:px-10">
                ...
                <div class="mt-5 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto_auto]">
                    ...
                    <label class="sr-only" for="library-sort">Ordenar biblioteca</label><select id="library-sort" :value="library.filters.sort" class="h-11 border border-border bg-card px-3 text-sm font-semibold" @change="visit({ sort: $event.target.value })"><option value="newest">Mais recentes</option><option value="oldest">Mais antigas</option><option value="title_asc">Título A–Z</option><option value="title_desc">Título Z–A</option></select>
                </div>
                <div v-if="filtersOpen" class="mt-3 grid gap-3 border border-border bg-card p-4 sm:grid-cols-3">
                    <label class="text-xs font-semibold text-muted-foreground">Idioma<select :value="library.filters.language || ''" class="mt-1.5 h-10 w-full border border-border bg-background px-3 text-sm text-foreground" @change="visit({ language: $event.target.value || null })"><option value="">Todos</option><option v-for="language in library.languages" :key="language.code" :value="language.code">{{ language.label }}</option></select></label>
                    <label class="text-xs font-semibold text-muted-foreground">Origem<select :value="library.filters.source || ''" class="mt-1.5 h-10 w-full border border-border bg-background px-3 text-sm text-foreground" @change="visit({ source: $event.target.value || null })"><option value="">Todas</option><option value="manual">Legendas manuais</option><option value="automatic">Legendas automáticas</option></select></label>
                    <label class="text-xs font-semibold text-muted-foreground">Tag<select :value="library.filters.tag || ''" class="mt-1.5 h-10 w-full border border-border bg-background px-3 text-sm text-foreground" @change="visit({ tag: $event.target.value || null })"><option value="">Todas</option><option v-for="tag in library.tags" :key="tag.publicId" :value="tag.publicId">{{ tag.name }}</option></select></label>
                </div>
                ...
                <button type="button" class="mt-5 inline-flex h-10 items-center border border-border bg-card px-4 text-sm font-semibold lg:hidden" @click="organizationDialog.showModal()">Pastas e tags</button>
                <div class="mt-5 grid min-w-0 gap-6 lg:grid-cols-[220px_minmax(0,1fr)]">
                    <aside class="hidden min-w-0 border-r border-border pr-5 lg:block" aria-label="Organização da biblioteca">
                        ...
                        <div class="mt-6 flex items-center justify-between"><h2 class="text-[11px] font-bold uppercase tracking-[0.16em] text-muted-foreground">Pastas</h2><button type="button" class="text-xs font-semibold text-accent" @click="openResource('create-folder', null, $event.currentTarget)">+ Nova</button></div>
                        <ul class="mt-2 space-y-1"><li v-for="folder in library.folders" :key="folder.publicId" class="flex min-w-0 items-center"><button type="button" class="flex min-w-0 flex-1 items-center justify-between gap-2 border-l-2 border-transparent px-2 py-2 text-left text-sm hover:bg-muted" :class="library.filters.folder === folder.publicId ? 'border-l-accent bg-muted text-foreground' : ''" @click="visit({ folder: folder.publicId })"><span class="truncate">{{ folder.name }}</span><span>{{ folder.count }}</span></button><button type="button" class="px-1 text-foreground hover:text-accent" :aria-label="`Renomear pasta ${folder.name}`" @click="openResource('rename-folder', folder, $event.currentTarget)">✎</button><button type="button" class="px-1 text-destructive" :aria-label="`Excluir pasta ${folder.name}`" @click="openResource('delete-folder', folder, $event.currentTarget)">×</button></li></ul>
                        <div class="mt-6 flex items-center justify-between"><h2 class="text-[11px] font-bold uppercase tracking-[0.16em] text-muted-foreground">Tags</h2><button type="button" class="text-xs font-semibold text-accent" @click="openResource('create-tag', null, $event.currentTarget)">+ Nova</button></div>
                        <ul class="mt-2 space-y-1"><li v-for="tag in library.tags" :key="tag.publicId" class="flex min-w-0 items-center"><button type="button" class="min-w-0 flex-1 truncate border-l-2 border-transparent px-2 py-2 text-left text-sm hover:bg-muted" :class="library.filters.tag === tag.publicId ? 'border-l-accent bg-muted text-foreground' : ''" @click="visit({ tag: tag.publicId })">{{ tag.name }}</button><button type="button" class="px-1 text-foreground hover:text-accent" :aria-label="`Renomear tag ${tag.name}`" @click="openResource('rename-tag', tag, $event.currentTarget)">✎</button><button type="button" class="px-1 text-destructive" :aria-label="`Excluir tag ${tag.name}`" @click="openResource('delete-tag', tag, $event.currentTarget)">×</button></li></ul>
                    </aside>
                    <main class="min-w-0">
                        <div v-if="selectedCount" class="sticky top-16 z-30 mb-3 flex flex-wrap items-center gap-2 border border-accent bg-card p-3" aria-live="polite"><strong class="mr-auto text-sm">{{ selectedCount }} {{ selectedCount === 1 ? 'selecionada' : 'selecionadas' }}</strong><button type="button" class="h-9 border border-border px-3 text-xs font-semibold" @click="openAction('move', selected, $event.currentTarget)">Mover</button><button type="button" class="h-9 border border-border px-3 text-xs font-semibold" @click="openAction('tags', selected, $event.currentTarget)">Tags</button><button type="button" class="h-9 border border-border px-3 text-xs font-semibold text-destructive" @click="openAction('remove', selected, $event.currentTarget)">Remover</button><button type="button" class="h-9 px-3 text-xs font-semibold text-muted-foreground" @click="selected = []">Limpar seleção</button></div>
                        <div v-if="library.items.length" class="border border-border bg-card">
                            ...
                            <article v-for="item in library.items" :key="item.publicId" class="grid min-w-0 grid-cols-[20px_88px_minmax(0,1fr)_36px] items-center gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:grid-cols-[20px_112px_minmax(0,1fr)_110px_36px]">
                                ...
                                <div data-item-menu class="relative"><button type="button" class="flex h-9 w-9 items-center justify-center text-lg font-bold text-foreground hover:bg-muted" :aria-label="`Ações para ${item.title}`" aria-haspopup="menu" :aria-expanded="openMenuId === item.publicId" @click.stop="toggleMenu(item, $event.currentTarget)">⋯</button><div v-if="openMenuId === item.publicId" :data-menu="item.publicId" role="menu" class="absolute right-0 top-10 z-20 w-52 border border-border bg-card p-1 shadow-xl"><Link :href="item.showUrl" role="menuitem" class="flex min-h-10 items-center px-3 text-sm hover:bg-muted">Abrir</Link><button type="button" role="menuitem" class="flex min-h-10 w-full items-center px-3 text-left text-sm hover:bg-muted" @click="openAction('move', [item.publicId], $event.currentTarget)">Mover para pasta</button><button type="button" role="menuitem" class="flex min-h-10 w-full items-center px-3 text-left text-sm hover:bg-muted" @click="openAction('tags', [item.publicId], $event.currentTarget)">Gerenciar tags</button><button type="button" role="menuitem" class="flex min-h-10 w-full items-center px-3 text-left text-sm text-destructive hover:bg-muted" @click="openAction('remove', [item.publicId], $event.currentTarget)">Remover da biblioteca</button></div></div>
                            </article>
                        </div>
                        ...
                        <nav v-if="library.pagination.lastPage > 1" class="mt-6 flex items-center justify-between gap-3" aria-label="Paginação da biblioteca"><Link v-if="library.pagination.previousPageUrl" :href="library.pagination.previousPageUrl" preserve-scroll class="h-10 border border-border bg-card px-4 py-2 text-sm font-semibold" @click="selected = []">← Anterior</Link><span v-else></span><p class="text-xs text-muted-foreground">Página {{ library.pagination.currentPage }} de {{ library.pagination.lastPage }}</p><Link v-if="library.pagination.nextPageUrl" :href="library.pagination.nextPageUrl" preserve-scroll class="h-10 border border-border bg-card px-4 py-2 text-sm font-semibold" @click="selected = []">Próxima →</Link><span v-else></span></nav>
                    </main>
                </div>
            </div>
        </section>
        <dialog ref="organizationDialog" class="m-auto max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-md overflow-y-auto border border-border bg-card p-0 text-foreground backdrop:bg-black/70" aria-labelledby="organization-title"><div class="flex items-center justify-between border-b border-border p-4"><h2 id="organization-title" class="text-lg font-semibold">Pastas e tags</h2><button type="button" class="h-10 px-3" aria-label="Fechar pastas e tags" @click="organizationDialog.close()">×</button></div><div class="p-4"><button type="button" class="flex w-full justify-between py-3" @click="visit({ folder: null }); organizationDialog.close()"><strong>Todas</strong><span>{{ library.counts.all }}</span></button><button type="button" class="flex w-full justify-between py-3" @click="visit({ folder: 'none' }); organizationDialog.close()"><span>Sem pasta</span><span>{{ library.counts.unfiled }}</span></button><h3 class="mt-5 text-xs font-bold uppercase text-muted-foreground">Pastas</h3><button v-for="folder in library.folders" :key="folder.publicId" type="button" class="flex w-full justify-between border-b border-border py-3 text-left" @click="visit({ folder: folder.publicId }); organizationDialog.close()"><span>{{ folder.name }}</span><span>{{ folder.count }}</span></button><button type="button" class="mt-3 text-sm font-semibold text-accent" @click="organizationDialog.close(); openResource('create-folder', null, $event.currentTarget)">+ Nova pasta</button><h3 class="mt-7 text-xs font-bold uppercase text-muted-foreground">Tags</h3><button v-for="tag in library.tags" :key="tag.publicId" type="button" class="block w-full border-b border-border py-3 text-left" @click="visit({ tag: tag.publicId }); organizationDialog.close()">{{ tag.name }}</button><button type="button" class="mt-3 text-sm font-semibold text-accent" @click="organizationDialog.close(); openResource('create-tag', null, $event.currentTarget)">+ Nova tag</button></div></dialog>
        <dialog ref="resourceDialog" class="m-auto w-[calc(100%-2rem)] max-w-md border border-border bg-card p-0 text-foreground shadow-2xl backdrop:bg-black/70" aria-labelledby="resource-dialog-title" @close="restoreDialogFocus"><form class="p-6" @submit.prevent="submitResource"><h2 id="resource-dialog-title" class="text-xl font-semibold">{{ resourceLabels[0] }}</h2><template v-if="!resourceMode.startsWith('delete')"><label for="resource-name" class="mt-5 block text-sm font-semibold">{{ resourceLabels[1] }}</label><input id="resource-name" ref="nameInput" v-model="resourceForm.name" maxlength="100" class="mt-2 h-11 w-full border border-border bg-background px-3 text-sm" /><p v-if="resourceForm.errors.name" class="mt-2 text-sm text-destructive">{{ resourceForm.errors.name }}</p></template><p v-else class="mt-4 text-sm leading-6 text-muted-foreground">{{ resourceMode === 'delete-folder' ? 'As transcrições permanecerão na biblioteca e irão para “Sem pasta”.' : 'A tag será removida das transcrições, mas nenhuma transcrição será apagada.' }}</p><div class="mt-6 flex justify-end gap-3"><button type="button" class="h-11 border border-border px-4 text-sm font-semibold" :disabled="resourceForm.processing" @click="resourceDialog.close()">Cancelar</button><button type="submit" class="h-11 bg-action px-4 text-sm font-semibold text-action-foreground disabled:opacity-60" :disabled="resourceForm.processing">{{ resourceForm.processing ? 'Salvando...' : resourceLabels[2] }}</button></div></form></dialog>
        <dialog ref="actionDialog" class="m-auto w-[calc(100%-2rem)] max-w-lg border border-border bg-card p-0 text-foreground shadow-2xl backdrop:bg-black/70" aria-labelledby="action-dialog-title" @close="restoreDialogFocus"><form class="p-6" @submit.prevent="submitAction"><h2 id="action-dialog-title" class="text-xl font-semibold">{{ actionMode === 'move' ? 'Mover para pasta' : actionMode === 'tags' ? 'Gerenciar tags' : `Remover ${actionItemIds.length} ${actionItemIds.length === 1 ? 'transcrição' : 'transcrições'} da biblioteca?` }}</h2><template v-if="actionMode === 'move'"><label for="move-folder" class="mt-5 block text-sm font-semibold">Destino</label><select id="move-folder" v-model="selectedFolder" class="mt-2 h-11 w-full border border-border bg-background px-3 text-sm"><option value="">Sem pasta</option><option v-for="folder in library.folders" :key="folder.publicId" :value="folder.publicId">{{ folder.name }}</option></select></template><template v-else-if="actionMode === 'tags'"><fieldset class="mt-5"><legend class="text-sm font-semibold">Ação</legend><div class="mt-2 flex gap-5"><label class="flex items-center gap-2 text-sm"><input v-model="tagOperation" type="radio" value="add" /> Adicionar tags</label><label class="flex items-center gap-2 text-sm"><input v-model="tagOperation" type="radio" value="remove" /> Remover tags</label></div></fieldset><fieldset class="mt-5 max-h-52 overflow-y-auto border border-border p-3"><legend class="px-1 text-sm font-semibold">Tags</legend><label v-for="tag in library.tags" :key="tag.publicId" class="flex min-h-10 items-center gap-3 text-sm"><input v-model="selectedTags" type="checkbox" :value="tag.publicId" /> {{ tag.name }}</label><p v-if="!library.tags.length" class="text-sm text-muted-foreground">Crie uma tag primeiro.</p></fieldset></template><p v-else class="mt-4 text-sm leading-6 text-muted-foreground">Os transcripts originais não serão apagados.</p><p v-if="actionForm.hasErrors" class="mt-3 text-sm text-destructive">Não foi possível concluir a ação. Revise a seleção.</p><div class="mt-6 flex justify-end gap-3"><button type="button" class="h-11 border border-border px-4 text-sm font-semibold" :disabled="actionForm.processing" @click="actionDialog.close()">Cancelar</button><button type="submit" class="h-11 bg-action px-4 text-sm font-semibold text-action-foreground disabled:opacity-60" :disabled="actionForm.processing || (actionMode === 'tags' && selectedTags.length === 0)">{{ actionForm.processing ? 'Processando...' : actionMode === 'remove' ? 'Remover' : 'Aplicar' }}</button></div></form></dialog>
    </PublicLayout>
</template>
